...
¿Qué función/es de R podríamos utilizar para conocer la cantidad de genes y de muestras a partir de esta tabla?
¿Cuántos genes hay?
¿Y muestras?
...
`r mycounter<-mycounter+1; mycounter`.
**¿A qué corresponde cada fila?** `r mycounter<-mycounter+1; mycounter`.
...
Por lo tanto, podemos considerar que cada muestra está en una determinada posición en cada uno de los `r n.genes` ejes definidos por el conjunto de genes.

![](./imagenes/coordinates.png){width="451"}
...
Exportamos las primeras filas de esta tabla a un archivo *.csv* que guardamos en el almacenamiento.
...
Ahora, con esta tabla de conteos normalizados podemos graficar los niveles de expresion de los `r topgenes.n` genes con mayores diferencias entre sexos.
...
Por ahora sólo queremos comparar la expresión de los `r topgenes.n` genes de interés entre los tejidos cefálicos de machos y hembras control.
